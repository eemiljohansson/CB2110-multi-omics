...
```{r}
# Load the data. The data was obtain from a TCGA study on breast cancer investigating miRNA, mRNA, and proteins.
miRNA <- read_csv("../data/miRNA.csv")
mRNA <- read_csv("../data/mRNA.csv")
protein <- read_csv("../data/protein.csv")
```

```{r}
# Let's check the dimensions of the data
tcga_data <- list(miRNA = miRNA, 
             mRNA = mRNA, 
             protein = protein)

lapply(tcga_data,dim)
```

Let's have a look at the dataframes

```{r}
print(tcga_data$miRNA)
print(tcga_data$mRNA)
print(tcga_data$protein)
```

We need to transfom the data so that the samples are in the columns and the features are in the rows. This is because MOFA expects the data to be in this format.

```{r}
swap_id_and_samples <- function(df, id_col = "ID") {
  # 1) extract the ID vector
  ids <- df[[id_col]]
  # 2) drop the ID column and transpose
  mat <- t(df[ , setdiff(names(df), id_col)])
  # 3) coerce back to a data.frame
  out <- as.data.frame(mat, stringsAsFactors = FALSE)
  # 4) assign the old IDs as column names
  colnames(out) <- ids
  # 5) assign the old variable names as row names
  rownames(out) <- setdiff(names(df), id_col)
  return(out)
}

# apply to each of your tcga_data elements:
tcga_data$miRNA   <- swap_id_and_samples(tcga_data$miRNA,   id_col = "ID")
tcga_data$mRNA    <- swap_id_and_samples(tcga_data$mRNA,    id_col = "ID")
tcga_data$protein <- swap_id_and_samples(tcga_data$protein, id_col = "ID")
...
```{r}
CLL_data$Drugs |> as.data.frame()
CLL_data$Mutations |> as.data.frame()
CLL_data$mRNA |> as.data.frame()
CLL_data$Methylation |> as.data.frame()
```
