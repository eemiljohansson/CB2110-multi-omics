...
[Link Google colab notebook.](https://colab.research.google.com/drive/1wFJIYDBkKr5JWjfctR5AJdnipKIil33r?usp=sharing)
...
The data we are using comes from a multi-omics study on COVID-19, published in The Journal of Cell Reports: ["**The COVIDome Explorer researcher portal**"](https://doi.org/10.1016/j.celrep.2021.109527) by Kelly Daniel Sullivan et al (2021). See paper for more information on the study methods.

In this session, we will integrate SOMAscan proteomics data with MSD Cytokine Profiles; and Sample Metadata. This data was collected from [Mendeley](https://doi.org/10.17632/2mc6rrc5j3.1).
...
ggsave("../plots/pdf/metadata_age_covid_status.pdf", width = 8, height = 6)
ggsave("../plots/png/metadata_age_covid_status.png", width = 8, height = 6)
...
ggsave("../plots/pdf/metadata_age_covid_distribution_status.pdf", width = 8, height = 6)
ggsave("../plots/png/metadata_age_distribution_covid_status.png", width = 8, height = 6)
...
ggsave("../plots/pdf/metadata_age_sex_covid_distribution_status.pdf", width = 8, height = 6)
ggsave("../plots/png/metadata_age_sex_distribution_covid_status.png", width = 8, height = 6)
```
...
pdf("../plots/pdf/factor_correlation.pdf", width = 8, height = 6)
plot_factor_cor(MOFAobject.trained)
dev.off()

png("../plots/png/factor_correlation.png", width = 800, height = 600)
plot_factor_cor(MOFAobject.trained)
dev.off()
```
...
pdf("../plots/pdf/variance_explained_per_view.pdf", width = 8, height = 6)
plot_variance_explained(MOFAobject.trained, x="view", y="factor")
dev.off()

png("../plots/png/variance_explained_per_view.png", width = 800, height = 600)
plot_variance_explained(MOFAobject.trained, x="view", y="factor")
dev.off()
```
...
```{r}
plot_variance_explained(MOFAobject.trained, x="group", y="factor", plot_total = TRUE)[[2]]

pdf("../plots/pdf/variance_explained_per_factor.pdf", width = 8, height = 6)
plot_variance_explained(MOFAobject.trained, x="group", y="factor", plot_total = TRUE)[[2]]
dev.off()

png("../plots/png/variance_explained_per_factor.png", width = 800, height = 600)
plot_variance_explained(MOFAobject.trained, x="group", y="factor", plot_total = TRUE)[[2]]
dev.off()
```
...
**By inspecting the factor value plots below, what can you tell about the factors? Do you see any patterns? Are there any factors that explain certain clinical variables more than others? What could be the reason for that?**
